...
void insert(void)
{
  ...
  // realloc inventory array
  if (num_parts == max_parts) {
    printf("Database is full; growing database.\n");
    max_parts = max_parts * 2;
    tmp = realloc(inventory, sizeof(struct part) * max_parts);
    if (tmp == NULL) {
        printf("can't grow database; failed to allocate memory\n");
        return;
    }
    inventory = tmp;
  }
...
/**********************************************************
 * compare_parts: compare parts for qsort                 *
 **********************************************************/
int compare_parts(const void *p1, const void *p2) {
